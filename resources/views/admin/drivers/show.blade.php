            ...
            <div class="d-flex flex-column flex-lg-row">
                ...
                <div class="flex-lg-row-fluid ms-lg-15">
                    <ul class="nav nav-custom nav-tabs nav-line-tabs nav-line-tabs-2x border-0 fs-4 fw-semibold mb-8" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <a class="nav-link text-active-primary pb-4 {{ session('current_process_number') ==3  ?'active ' :' ' }} " data-bs-toggle="tab" href="#licenses" aria-selected="true" role="tab">{{__("messages.licenses")}}</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link text-active-primary pb-4  {{ session('current_process_number') ==4  ?'active ' :' ' }} " data-bs-toggle="tab" href="#vehicle_information" aria-selected="true" role="tab">{{__("messages.vehicle_information")}}</a>
                        </li>
                        <li class="nav-item" role="presentation">
                    ...
                    <div class="tab-content" id="myTabContent">
                        ...
                        <div class="tab-pane fade {{session('current_process_number') ==0 || session('current_process_number') ==2  ?'active show' :' ' }}" id="official_documents" role="tabpanel">
                            ...
                            <div style="display: flex; align-items:center;justify-content:center" class="my-2 col-12">
                                ...
                                <form class="mx-1" method="post" action="{{ route('admin.drivers.change_process_status') }}">
                                    @csrf
                                    <input type="hidden" name="process" value="{{$process_number}}">
                        ...
                        <div class="tab-pane fade  {{ session('current_process_number') ==3  ?'active show' :' ' }}" id="licenses" role="tabpanel">
                            @php
                            $process_number = 3;
                            $process = $driver->processes->firstWhere('process_number', $process_number);
                            $status = $process ? $process->status : null;
                            ...
                            <div class="card pt-4 mb-6 mb-xl-9">
                                ...
                                <div class="card-body pt-0 pb-5">
                                    <div id="kt_table_customers_payment_wrapper" class="dt-container dt-bootstrap5 dt-empty-footer">
                                        <div class="row mb-7">
                                            <label class="col-lg-4 fw-semibold text-muted">{{__("messages.driving_license_issue_date")}}</label>

                                            <div class="col-lg-8">
                                                <span class="fw-bold fs-6 text-gray-800">{{$driver->driving_license_issue_date}}</span>
                                            </div>
                                            <label class="col-lg-4 fw-semibold text-muted">{{__("messages.Expire")}}</label>

                                            <div class="col-lg-8">
                                                <span class="fw-bold fs-6 text-gray-800">{{$driver->driving_license_expiry_date}}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card pt-4 mb-6 mb-xl-9">
                                ...
                                <div id="kt_customer_view_payment_method" class="card-body pt-0">
                                    ...
                                    @foreach($groupedDocuments[3] as $document)
                                    <div class="py-0" data-kt-customer-payment-method="row">
                                        <div class="py-3 d-flex flex-stack flex-wrap">
                                            <div class="d-flex align-items-center collapsible rotate collapsed" data-bs-toggle="collapse" href="#{{$document->id}}" role="button" aria-expanded="false" aria-controls="{{$document->id}}">
                                                <div class="me-3 rotate-90"><i class="ki-duotone ki-right fs-3"></i></div>
                            ...
                            <div style="display: flex; align-items:center;justify-content:center" class="my-2 col-12">
                                ...
                                @if($status === 'accepted' || $status =="pending")
                                <button type="submit" onclick="setProcessNumber({{$process_number}})" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#rejected-reson">
                                    <span style="margin-right: 10px;">&#10006;</span>{{ __('messages.Reject') }}
                                </button>
                                @endif
                        ...
                        <div class="tab-pane fade  {{ session('current_process_number') ==4  ?'active show' :' ' }}" id="vehicle_information" role="tabpanel">
                            @php
                            $process_number = 4;
                            $process = $driver->processes->firstWhere('process_number', $process_number);
                            $status = $process ? $process->status : null;
                                ...
                                <div class="card-body pt-0 pb-5">
                                    <div id="kt_table_customers_payment_wrapper" class="dt-container dt-bootstrap5 dt-empty-footer">
                                        <div class="row mb-7">
                                            ...
                                            <div class="col-lg-8">
                                                <span class="fw-bold fs-6 text-gray-800">{{$driver->car_license_expiration}}</span>
                                            </div>
                                            <label class="col-lg-4 fw-semibold text-muted">{{__("messages.Car_type")}}</label>
                                            <div class="col-lg-8">
                                                <span class="fw-bold fs-6 text-gray-800">{{session('lang') == "en" ?$driver->carType->name : $driver->carType->name_ar}}</span>
                                            </div>

                                            <label class="col-lg-4 fw-semibold text-muted">{{__("messages.Brand")}}</label>
                                            <div class="col-lg-8">
                                                <span class="fw-bold fs-6 text-gray-800">{{session('lang') == "en" ?$driver->brand->name : $driver->brand->name_ar}}</span>
                                            </div>

                                            <label class="col-lg-4 fw-semibold text-muted">{{__("messages.Model")}}</label>
                                            <div class="col-lg-8">
                                                <span class="fw-bold fs-6 text-gray-800">{{session('lang') == "en" ?$driver->model->name : $driver->model->name_ar}}</span>
                                            </div>

                                            <label class="col-lg-4 fw-semibold text-muted">{{__("messages.production_year")}}</label>
                                            <div class="col-lg-8">
                                                <span class="fw-bold fs-6 text-gray-800">{{$driver->production_year}}</span>
                                            </div>

                                            <label class="col-lg-4 fw-semibold text-muted">{{__("messages.car_color")}}</label>
                                            <div class="col-lg-8">
                                                <span class="fw-bold fs-6 text-gray-800">{{$driver->car_color}}</span>
                                            </div>

                                            <label class="col-lg-4 fw-semibold text-muted">{{__("messages.car_plate_number")}}</label>
                                            <div class="col-lg-8">
                                                <span class="fw-bold fs-6 text-gray-800">{{$driver->car_plate_number}}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card pt-4 mb-6 mb-xl-9">
                                ...
                                <div id="kt_customer_view_payment_method" class="card-body pt-0">
                                    ...
                                    @foreach($groupedDocuments[4] as $document)
                                    <div class="py-0" data-kt-customer-payment-method="row">
                                        <div class="py-3 d-flex flex-stack flex-wrap">
                                            <div class="d-flex align-items-center collapsible rotate collapsed" data-bs-toggle="collapse" href="#{{$document->id}}" role="button" aria-expanded="false" aria-controls="{{$document->id}}">
                                                <div class="me-3 rotate-90"><i class="ki-duotone ki-right fs-3"></i></div>
...
<div class="img-popup">
    <img src="" alt="Popup Image" style="    height: 400px;width: auto;">
    <div class="close-btn">
        <div class="bar"></div>
        <div class="bar"></div>
    </div>
