            ...
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">
                        {{ __('messages.create_handyman') }}</h1>
                ...
                <div class="flex-column flex-lg-row-auto w-100  mb-10">
                    ...
                    <div class="card mb-5 mb-xl-8">
                        ...
                        <div class="card-body pt-15">
                            <form action="{{ route('admin.handymans.store') }}" method="POST"
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="tel" name="phone" class="form-control" maxlength="11" pattern="\d*"
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="id_number" class="form-control" required pattern="\d*"
                                ...
                                <!-- اختيار القسم -->
                                <div class="mb-3">
                                    <label class="form-label"> {{ __("messages.Choose_section") }}</label>
                                    <select id="sectionSelect" class="form-select" required>
                                        <option value="" disabled>-- {{ __("messages.Choose_section") }}--</option>
                                        <option value="car_services"> {{ __("messages.car_services") }}</option>
                                        <option value="home_services">{{ __("messages.home_services") }} </option>
                                    </select>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select id="serviceSelect" name="services[]" class="form-select" multiple required>
                                        {{-- كل الخدمات، وسنخفي بعضها باستخدام JS --}}
                                        @foreach ($services as $service)
                                            <option value="{{ $service->id }}" data-section="{{ $service->section }}">
                                                {{ $service->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <small class="text-muted">@lang('messages.Click_on_anything_to_get_more_from_the_service.')</small>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="file" name="criminal_record" class="form-control" accept="image/*"
                                        required>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label"> @lang('messages.id_front') </label>
                                    <input type="file" name="id_front" class="form-control" accept="image/*"
                                        required>
                                </div>
...
<script>
    document.getElementById('sectionSelect').addEventListener('change', function () {
        ...
        const serviceOptions = document.querySelectorAll('#serviceSelect option');

        serviceOptions.forEach(option => {
            if (option.dataset.section === selectedSection) {
